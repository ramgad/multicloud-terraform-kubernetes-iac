...
display_help() {
  ...
  echo "$(date) -->  VM         for example: ./setup_env.sh vm cluster01 rg-lab-01"
  echo "$(date) -->  BareMetal  for example: ./setup_env.sh baremetal cluster01 rg-lab-01"
  echo
  exit 1
}
...
if [[ $CLOUD_ENV == "aws" ]]; then
  ...
  SERVICE_PRINCIPAL_JSON=$(az ad sp create-for-rbac --skip-assignment --name rg-aks-service-account -o json)
