...
We will use Terraform + Kubespray to set up the Kubernetes cluster with k8sproject on these machines.

### Pre-requisites:

- Machines running with CentOS or Ubuntu and the configuration must meet the minimum [requirements for k8sproject](https://docs.k8sproject.com/start-here-installation/). k8sproject requires minimum 3 worker nodes to run. All machines must have 4CPUs and 4GB of RAM.
