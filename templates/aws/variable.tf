...
variable "SpearAhead_env" {
   description = "Enter the SpearAhead Environment name (dev, stg, tst, prd):"
   default = "dev"
}

variable "SpearAhead_aws_keypair" {
...
variable "px_cloud_storage_type" {
   default = "gp2"
   description = "Enter the k8sproject storage type gp2/ssd"
}
...
variable "px_kvdb_device_storage_type" {
   default = "gp2"
   description = "Enter the k8sproject kvdb device storage type gp2/ssd"
}
