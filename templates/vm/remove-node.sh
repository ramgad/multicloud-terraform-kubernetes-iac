...
#  timeout 25 ssh -oBatchMode=yes ${PX_ANSIBLE_USER}@${HOST_IP} "sudo hostname" || { printf "\nSSH Connection failed: ${PX_ANSIBLE_USER}@${HOST_IP}\n\nMake sure the host is reachable and password-less ssh is set up correctly.\n\n"; exit 1; } 
  ...
  printf "Find a k8sproject pod: " >> "${LOG_FILE}"
  PX_POD="$(${kbCtl} --kubeconfig="../${vKubeConfig}" get pods --no-headers -l name=k8sproject -n k8sproject -o wide | grep -v ${HOST_TO_REMOVE} | xargs | cut -f1 -d' ')"
  printf "${PX_POD}\n" >> "${LOG_FILE}"
  ...
  printf "Checking if node is a member of the k8sproject cluster: " >> "${LOG_FILE}"
  PX_NODE="$(${kbCtl} --kubeconfig="../${vKubeConfig}" exec -n k8sproject $PX_POD -c k8sproject -- /opt/pwx/bin/pxctl cluster list 2>/dev/null | grep -A100 "Nodes in the cluster:"| grep "${HOST_TO_REMOVE}" |xargs | cut -f2 -d' ' || true)"
  if [[ "${PX_NODE}" != "" ]]; then
    printf -- "Yes!\n" >> "${LOG_FILE}"
    printf "\n"
    printf "Preparing node for removal...\n" | tee -a "${LOG_FILE}"

    printf "Putting k8sproject node in maintenance mode: " >> "${LOG_FILE}"
    ...
    printf "Setting label on the node to remove k8sproject from the host.\n" >> "${LOG_FILE}"
    ${kbCtl} --kubeconfig="../${vKubeConfig}" label nodes ${HOST_TO_REMOVE} px/enabled=false --overwrite  > /dev/null 2>&1
    
    printf "Getting k8sproject node UID: " >> "${LOG_FILE}"
    NODE_UID="$(${kbCtl} --kubeconfig="../${vKubeConfig}" get storagenodes.core.libopenstorage.org ${HOST_TO_REMOVE} -n k8sproject -o jsonpath={.status.nodeUid} 2>/dev/null)" || true
    printf "${NODE_UID}\n" >> "${LOG_FILE}"
    sleep 10

    printf "Cleaning k8sproject node before removal: " >> "${LOG_FILE}"
    timeout 100 ssh -oBatchMode=yes ${PX_ANSIBLE_USER}@${HOST_IP} "
        sudo systemctl stop k8sproject
        sudo systemctl disable k8sproject
        sudo rm -f /etc/systemd/system/k8sproject*
        grep -q '/opt/pwx/oci /opt/pwx/oci' /proc/self/mountinfo && sudo umount /opt/pwx/oci
        pxctl service node-wipe --all
        " >/dev/null 2>&1 || true
    printf -- "Finished!\n" >> "${LOG_FILE}"

    printf "Removing node from the k8sproject storage cluster if available: " >> "${LOG_FILE}"
    ${kbCtl} --kubeconfig="../${vKubeConfig}" exec -n k8sproject $PX_POD -c k8sproject -- /opt/pwx/bin/pxctl cluster delete ${NODE_UID} >>"${LOG_FILE}" 2>&1 || true
    printf -- "Done!\n" >> "${LOG_FILE}"
  else
    printf -- "No!\n" >> "${LOG_FILE}"
  fi
